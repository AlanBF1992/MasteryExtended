    ...
    internal static class MasteryTrackerMenuPatch
    {
        ...
        internal static void drawPostfix(MasteryTrackerMenu __instance, SpriteBatch b, ClickableTextureComponent __state)
        {
            ...
            if (__instance.mainButton?.myAlternateID == 99)
            {
                ...
                Utility.drawTextWithColoredShadow(b, text, Game1.dialogueFont, __instance.mainButton.getVector2() + new Vector2((float)(__instance.mainButton.bounds.Width / 2) - Game1.dialogueFont.MeasureString(text).X / 2f, 6f + (float)((__instance.mainButton.sourceRect.X == 84) ? 8 : 0)), Color.Black, Color.Black * 0.2f, 1f, 0.9f);
            }
